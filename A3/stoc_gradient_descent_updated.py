...
def sgd(x, y, rate, num_iter,lam,Islasso):
    '''
    ...
    for j in range(0,num_iter):
        ...
        diff=xi.dot(beta)-yi
        if(Islasso):
            beta = beta-rate*(xi.T.dot(diff) + lam*np.sign(beta))
        else:
            beta = beta-rate*(xi.T.dot(diff) + 2*lam*beta)
        count=count+1
        costs.append(error(y,x.dot(beta)))
        if(count%50==0):
...
def process(data, learning_rate, num_iter,degree,Islasso):
    ...
    for j in np.arange(0.1, 1.1, 0.1):
        ...
        beta = sgd(x, y_org, learning_rate, num_iter,j,1)
